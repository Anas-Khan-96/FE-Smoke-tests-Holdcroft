/**
 * Test Case: Verify that the 'Make' search filters function correctly
 * 
 * Given: The user is on the Holdcroft Vehicle Search Page (VSP)
 * When: The user selects a 'Make' from the search filters and clicks on 'Update Results'
 * Then: The VSP is updated to display only vehicles of the selected make
 */

describe('Verify that the \'Make\' search filters function correctly', () => {
  it('passes', () => {
    // Visit the page
    ...
    // Expected Outcome: The VSP is updated to display only vehicles of the selected make
    cy.get('.cars-container').should('be.visible')
    cy.get('#vehicle-card-0 > .content-container > .prices').should('be.visible')
    cy.get('#CX17NWM > .make').should('contain', 'Audi')
  })
})